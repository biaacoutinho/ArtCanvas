...
namespace Grafico
{
    public class ListaSimples<Dado> where Dado : IComparable<Dado>, IRegistro
    {
        ...
        internal void Adicionar(Dado novoDado)
        {
            if (EstaVazia)
            {
                InserirAntesDoInicio(novoDado);
            }
            else if (novoDado.CompareTo(primeiro.Info) < 0)
                InserirAntesDoInicio(novoDado);
            else if (novoDado.CompareTo(ultimo.Info) > 0)
                InserirAposFim(novoDado);
            else
            {
                anterior = atual = null;
                atual = primeiro;
                while (atual != null && atual.Info.CompareTo(novoDado) < 0)
                {
                    anterior = atual;
                    atual = atual.Prox;
                }
